...
### 5.2 Global SHAP
- [FILL] Globally, the model relies most on: <top-5 features from cell above>
- Capital-gain and capital-loss show high magnitude but sparse signal (most values = 0);
  when non-zero they dominate prediction.
- Education-num, hours-per-week, and marital-status are the most consistent drivers.

### 5.3 Subgroup Errors
- [FILL] False negatives concentrate in: <group from error_tables>
- [FILL] False positives concentrate in: <group from error_tables>
- [FILL] Largest FNR gap by sex: Male FNR=X.XX vs Female FNR=X.XX (Δ=X.XX)
- [FILL] Largest selection rate gap by race: <group>=X.XX vs <group>=X.XX

### 5.4 Local (Case Studies)
- FP: High hours + exec occupation → model over-predicts >50K; true income ≤50K
  likely because capital-gain/loss = 0 (not captured in profile).
- FN: Correct demographic profile but model misses due to low education-num.

### 5.5 Counterfactuals
- [FILL] FN flip: raising hours-per-week to 50 [did/did not] flip prediction
  (prob X.XX → X.XX).
- [FILL] FN flip: raising education-num to 14 [did/did not] flip prediction.
- Minimum single-feature flip found: [FILL feature + value].

### 5.6 Fairness Cross-link
- SHAP importance for 'sex' [does/does not] differ markedly between Male and Female
  subgroups — marital-status is more predictive for Female, hours-per-week for Male.
- SHAP ≠ causal; 1994 census data; model should not inform real decisions.

### Monitoring recommendations
- Track FPR and FNR by sex and race on any new inference batch.
- Monitor drift on top-3 SHAP features: [FILL features] — distribution shift
  in these features is the earliest signal of model degradation.
- Re-evaluate fairness metrics quarterly if model is ever deployed live.
